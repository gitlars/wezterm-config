...
local pane_resize = require 'modules.pane_resize'
...
local SPEC = {
  ...
  { group = 'Panes', desc = 'equalize the current row of panes', label = 'LEADER CTRL+=',
    key = '=', mods = 'LEADER|CTRL', action = wezterm.action_callback(pane_resize.equalize) },
  ...
  -- activate -> resize -> verify pattern as equalize (modules/pane_resize.lua),
  -- but rings the bell rather than showing a toast on failure -- these are
  -- meant to be pressed repeatedly while exploring a layout, and a toast
  -- on every wezterm#7401 miss is too heavy for that.
  { group = 'Panes', desc = 'grow the active pane to the right by 1 cell', label = 'CTRL+→',
    key = 'RightArrow', mods = 'CTRL', action = wezterm.action_callback(function(window, pane)
      ...
      local ok = pane_resize.resize_verified(window, tab, pane:pane_id(), 'Right', 1)
      if not ok then pane_resize.ring_bell(pane) end
    end) },
  { group = 'Panes', desc = 'shrink the active pane from the right by 1 cell', label = 'CTRL+←',
    key = 'LeftArrow', mods = 'CTRL', action = wezterm.action_callback(function(window, pane)
      local tab = pane:tab()
      if not tab then return end
      local ok = pane_resize.resize_verified(window, tab, pane:pane_id(), 'Left', 1)
      if not ok then pane_resize.ring_bell(pane) end
    end) },
  { group = 'Panes', desc = 'grow the active pane toward the bottom by 1 cell', label = 'CTRL+↓',
    key = 'DownArrow', mods = 'CTRL', action = wezterm.action_callback(function(window, pane)
      local tab = pane:tab()
      if not tab then return end
      local ok = pane_resize.resize_verified(window, tab, pane:pane_id(), 'Down', 1)
      if not ok then pane_resize.ring_bell(pane) end
    end) },
  { group = 'Panes', desc = 'shrink the active pane from the bottom by 1 cell', label = 'CTRL+↑',
    key = 'UpArrow', mods = 'CTRL', action = wezterm.action_callback(function(window, pane)
      local tab = pane:tab()
      if not tab then return end
      local ok = pane_resize.resize_verified(window, tab, pane:pane_id(), 'Up', 1)
      if not ok then pane_resize.ring_bell(pane) end
    end) },
